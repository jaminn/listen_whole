                ...
                if (getFuture() < How_video_pasted) { //진행시간이 미래 보다 크면
                    ...
                    if (getNow() <= How_video_pasted) { //진행시간이 미래 보다 작고, 현재보다 크면
                        ...
                        [0, 1, 2, 3].forEach(function (my_x){
                            $("#buttonP").append('<div class="center-block text-center container" style="float:left;left:10%"><button class="btn btn-primary btn-lg center-block" style="color:#000;font-size:17px;position: relative; display: block;vertical-align: middle; height:80px;width:105px; margin:10px 10px;" onclick="my_dis(this)">' + SuffButton[my_x] + "</button></div>");
                        });
                        $("#ad").html("");
                        nxtWhere();
